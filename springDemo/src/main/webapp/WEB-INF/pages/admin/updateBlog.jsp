...
<body>
	<div class="container">
		...
		<form:form action="../updateBlog" method="post" commandName="blogP"
			...
			<div class="form-group">
				<label for="userId">Author:</label> <select class="form-control"
					id="userId" name="userId">
					<c:forEach items="${userList}" var="user">
						<c:if test="${user.id==blog.user.id}">
							<option value="${user.id}" selected="selected">${user.nickname},
								${user.firstName} ${user.lastName}</option>
						</c:if>
						<c:if test="${user.id!=blog.user.id}">
							<option value="${user.id}">${user.nickname},
								${user.firstName} ${user.lastName}</option>
						</c:if>
					</c:forEach>
				</select>
			...
			<div class="form-group">
			 ...
			 <div class="form-group">
				<label for="pubDate">Publish Date:</label> <input type="date"
					class="form-control" id="pubDate" name="pubDate"
