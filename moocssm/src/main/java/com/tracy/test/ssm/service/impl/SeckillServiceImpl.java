package com.tracy.test.ssm.service.impl;
...
import com.tracy.test.ssm.dao.SeckillDao;
import com.tracy.test.ssm.dao.SuccessKilledlDao;
import com.tracy.test.ssm.dao.cache.RedisDao;
...
import com.tracy.test.ssm.service.SeckillService;
...
public class SeckillServiceImpl implements SeckillService {
	...
	// md5盐值字符串，用于混淆md5
	private final String slat = "fgdfg&(*34236^&%*";
	...
	public SeckillExecution excuteSeckill(long seckillId, long userPhone,
		...
		try {
			// 记录购买行为
			int insertCount = successKilledlDao.insertSuccessKilled(seckillId,
					userPhone);
			if (insertCount <= 0) {
				...
				if (updateCount <= 0) {
					...
					// 秒杀成功, commit
					SuccessKilled successKilled = successKilledlDao
	...
	@Override
	public SeckillExecution excuteSeckillProcedure(long seckillId,
		...
		try {
			...
			if (result == 1) {
				SuccessKilled sk = successKilledlDao.queryByIdWithSeckill(
						seckillId, userPhone);
				return new SeckillExecution(seckillId,
